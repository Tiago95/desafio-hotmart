...
@Entity
@Table(name = "usuario")
@NamedNativeQuery(name = "SqlGetContatosVOByUsuario", query = "SELECT u.id AS id, u.nome AS nome, u.nick_name AS nick, CASE WHEN ub.id IS NOT NULL THEN 'true' ELSE 'false' END AS bloqueado, CASE WHEN pa.id IS NOT NULL THEN pa.status_solicitacao ELSE 0 END AS statusSolicitacaoAmizade, CASE WHEN pa.id IS NOT NULL AND pa.usuario_principal_id = :idUsuario THEN 'true' ELSE 'false' END AS amizadeSolicitada, u.permitir_conversas_anonimas AS permitirConversasAnonimas, u.conectado AS conectado FROM usuario u LEFT JOIN usuario_bloqueado ub ON (ub.usuario_principal_id = :idUsuario AND ub.usuario_bloqueado_id = u.id) LEFT JOIN pedido_amizade pa ON ((pa.usuario_principal_id = :idUsuario AND pa.usuario_amigo_id = u.id) OR (pa.usuario_amigo_id = :idUsuario AND pa.usuario_principal_id = u.id)) WHERE u.id <> :idUsuario", resultSetMapping = "SqlGetContatosVOByUsuario")
@SqlResultSetMapping(name = "SqlGetContatosVOByUsuario", classes = @ConstructorResult(targetClass = ContatosVO.class, columns = {@ColumnResult(name = "id", type = Long.class), @ColumnResult(name = "nome", type = String.class), @ColumnResult(name = "nick", type = String.class), @ColumnResult(name = "bloqueado", type = Boolean.class), @ColumnResult(name = "statusSolicitacaoAmizade", type = Integer.class), @ColumnResult(name = "amizadeSolicitada", type = Boolean.class), @ColumnResult(name = "permitirConversasAnonimas", type = Boolean.class), @ColumnResult(name = "conectado", type = Boolean.class)}))
public class Usuario extends BaseEntity implements UserDetails {
	...
	public UsuarioVO toUsuarioVO() {
		...
		usuarioVO.setPermitirConversasAnonimas(this.isPermitirConversasAnonimas());
